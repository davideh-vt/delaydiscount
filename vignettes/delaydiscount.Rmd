...
Our model assumes that the transformed observed indifference point will follow the model

$\log(\frac{1}{\tilde D(t_{ijc})} - 1) = \log(k_{ic}) + \log(t_{ijc}) + \epsilon$

Where $\epsilon \tilde N(0, \sigma^2)$, $i$ is an index for subject, $j$ an index for time point, and $c$ is an index for group.

(TODO: Background citation for the linearized model.)
